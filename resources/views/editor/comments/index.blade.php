...
<div x-data="{
    ...
    matches(comment) {
        ...
        return comment.article.toLowerCase().includes(searchTerm) ||
    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="mb-8">
            ...
            <p class="text-gray-600 dark:text-gray-400">Kelola komentar pada artikel Anda</p>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 mb-6">
            <div class="relative w-full sm:w-80">
                ...
                <input type="text" x-model="search" placeholder="Cari artikel, user, atau komentar..."
                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse ($comments as $comment)
                            <tr x-show="matches({
                                ...
                                user: '{{ addslashes($comment->user->name) }}',
                                body: '{{ addslashes(Str::limit($comment->body, 50)) }}'
                                ...
                                <td class="px-6 py-4">
                                    <div class="text-sm text-gray-900 dark:text-white max-w-md">
                                        <p class="line-clamp-2">{{ Str::limit($comment->body, 50) }}</p>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 text-center">
                                    <form method="POST" action="{{ route('editor.comments.destroy', $comment->id) }}" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center justify-center space-y-3">
                                        ...
                                        <p class="text-gray-500 dark:text-gray-400 font-medium">Tidak ada komentar</p>
                                        <p class="text-sm text-gray-400 dark:text-gray-500">Komentar akan muncul saat ada yang berkomentar pada artikel Anda</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
